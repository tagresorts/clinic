    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('revenue.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium rounded-md transition-colors duration-200">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                    </svg>
                    Back to Dashboard
                </a>
                <a href="{{ route('revenue.export', ['start_date' => $startDate->format('Y-m-d'), 'end_date' => $endDate->format('Y-m-d')]) }}" class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md transition-colors duration-200">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <form method="GET" action="{{ route('revenue.reports') }}" class="flex items-center space-x-4">
                        <div>
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="text-center">
                            <p class="text-2xl font-bold text-green-600">${{ number_format($revenue, 2) }}</p>
                            <p class="text-sm text-gray-500">Total Revenue</p>
                        </div>
                        <div class="text-center">
                            <p class="text-2xl font-bold text-blue-600">{{ $dailyRevenue->count() }}</p>
                            <p class="text-sm text-gray-500">Days with Revenue</p>
                        </div>
                        <div class="text-center">
                            <p class="text-2xl font-bold text-purple-600">${{ $dailyRevenue->count() > 0 ? number_format($revenue / $dailyRevenue->count(), 2) : '0.00' }}</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Top Patients by Revenue</h3>
                        <div class="space-y-3">
                            @forelse($topPatients as $patient)
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                    ...
                                    <div class="text-right">
                                        <p class="text-sm font-medium text-gray-900">${{ number_format($patient->total_revenue, 2) }}</p>
                                    </div>
                                </div>
                            @empty
                        ...
                        <div class="space-y-3">
                            @forelse($outstandingInvoices->take(10) as $invoice)
                                <div class="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                                    ...
                                    <div class="text-right">
                                        <p class="text-sm font-medium text-gray-900">${{ number_format($invoice->outstanding_balance, 2) }}</p>
            ...
            @if($dailyRevenue->count() > 0)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-6">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Daily Revenue Trend</h3>
                        <div class="space-y-2">
                            @foreach($dailyRevenue as $day)
                                <div class="flex items-center justify-between p-2 bg-gray-50 rounded">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <span class="text-sm font-medium text-gray-900">${{ number_format($day->revenue, 2) }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
