...
class SendExpiringItemsAlerts extends Command
{
    protected $signature = 'stock:send-expiring-items-alerts';
    protected $description = 'Send digest email for expiring items';

    public function handle(): int
    {
        $reminderDays = Setting::where('key', 'expiring_items_reminder_days')->first()->value;
        $today = Carbon::today();

        $expiring = InventoryItem::where('has_expiry', true)->whereDate('expiry_date', '<=', $today->copy()->addDays($reminderDays))->get(['item_name','expiry_date'])->toArray();

        if (empty($expiring)) {
            $this->info('No expiring items alerts to send.');
            return self::SUCCESS;
        }
        ...
        $this->withSmtp(function () use ($recipients, $expiring) {
            foreach ($recipients as $email) {
                Mail::to($email)->queue(new StockDigestMail([], $expiring));
            }
        });

        $this->info('Expiring items digest queued to '.count($recipients).' recipient(s).');
        return self::SUCCESS;
    }
